...
*/
$c[$p]['ingame']['message'] = "<NICKNAME> tikko nopirka VIP statusu izmantojot SMS veikalu!";
...
$c['lang'][$p]['lv'] = array(
    "instructions" => "Lai iegādātos VIP uz <LENGTH> dienām par <PRICE> EUR, sūti kodu <b><KEYWORD><CODE></b> uz <b><NUMBER></b>, lai saņemtu atslēgas kodu!",
	# Kļūdas
    ...
    "money_purchased" => "VIP veiksmīgi iegādāts. Lai jauka spēlēšana!",
	# Forma
    "form_price" => "Cena",
    ...
    "table_flags" => "Flagi",
    "table_date" => "Datums",
    "table_no_buyers" => "Neviens vēl nav iegādājies VIP. Varbūt vēlies būt pirmais?"
);

$c['lang'][$p]['en'] = array(
	"instructions" => "To purchase <LENGTH> days VIP for <PRICE> EUR, send the following code: <b><KEYWORD><CODE></b> to <b><NUMBER></b> to receive an unclock code!",
	# Kļūdas
	...
	"money_purchased" => "VIP was purchased successfully. Have fun!",
	# Forma
	"form_price" => "Price",
    ...
    "table_flags" => "Flags",
    "table_date" => "Date",
    "table_no_buyers" => "No one has bought VIP yet. Would you like to be the first?"
);
/*
-----------------------------------------------------
    Minecraft naudas spraudņa konfigurācija
...
if(isset($_POST['code'])):
	...
	if(count($errors) > 0){
		...
		if($baltsms->getResponse() === true){
			...
			$amxclass->send_chat($_POST['server'], 'Lūdzu, ienāc pa jaunam serverī, lai VIP privilēģijas stātos spēkā!', 'amx_psay "' . $_POST['nickname'] . '"');
