...
/*
-----------------------------------------------------
    SAMP EXP naudas spraudņa konfigurācija
-----------------------------------------------------
*/
...
*/
$c[$p]['db']['table'] = "baltsms_samp_exp";

/*
    Vai uzrādīt pircēju sarakstu - jā/nē - true/false
*/
$c[$p]['sms']['buyers'] = false;
...
$c['lang'][$p]['lv'] = array(
    "instructions" => "Lai iegādātos <LENGTH> EXP par <PRICE> EUR, sūti kodu <b><KEYWORD><CODE></b> uz <b><NUMBER></b>, lai saņemtu atslēgas kodu!<br>Ar PayPal var maksat UCP paneli<br><span style='color:red'>Tu nedrīksti atrasties spēlē pasūtot pakalpojumu!</span>",
	# Kļūdas
    ...
    "money_purchased" => "EXP veiksmīgi iegādāts. Lai jauka spēlēšana!",
	# Forma
    "form_price" => "Cena",
...
/*
-----------------------------------------------------
    SAMP EXP spraudņa konfigurācija
-----------------------------------------------------
*/
...
if(isset($_POST['code'])):
	...
	if(count($errors) > 0){
		...
		if($baltsms->getResponse() === true){
			...
			$db->update("UPDATE `accounts` SET `exp` = `exp`+'".$c[$p]['prices'][$_POST['server']][$_POST['price']]."' WHERE `name` = '".$_POST['nickname']."'");
				...
				<?php foreach($c[$p]['prices'] as $server => $prices): ?>
					<select class="form-control prices" name="price" id="none-<?php echo $server; ?>-prices" style="display: none;" onChange="changePrice(this); getvalue(this);" disabled>
						...
						<?php foreach($prices as $price_code => $exp): ?>
							<option value="<?php echo $price_code; ?>" data-length="<?php echo $exp; ?>"><?php echo $exp; ?> - <?php echo baltsms::returnPrice($price_code); ?> EUR</option>
						<?php endforeach; ?>
					</select>
				<?php endforeach;  ?>
			</div>
